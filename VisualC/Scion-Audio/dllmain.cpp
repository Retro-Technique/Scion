...
static AFX_EXTENSION_MODULE ScionAudioDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de Scion-Audio.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(ScionAudioDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ScionAudioDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de Scion-Audio.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(ScionAudioDLL);
	}
	return 1;   // ok
}
