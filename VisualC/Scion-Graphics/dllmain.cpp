...
static AFX_EXTENSION_MODULE ScionGraphicsDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de Scion-Graphics.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(ScionGraphicsDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ScionGraphicsDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de Scion-Graphics.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(ScionGraphicsDLL);
	}
	return 1;   // ok
}
