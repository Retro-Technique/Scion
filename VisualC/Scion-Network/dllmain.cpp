...
static AFX_EXTENSION_MODULE ScionNetworkDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de Scion-Network.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(ScionNetworkDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ScionNetworkDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de Scion-Network.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(ScionNetworkDLL);
	}
	return 1;   // ok
}
