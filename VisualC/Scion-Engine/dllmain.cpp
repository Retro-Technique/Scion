...
static AFX_EXTENSION_MODULE ScionEngineDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de Scion-Engine.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(ScionEngineDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ScionEngineDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de Scion-Engine.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(ScionEngineDLL);
	}
	return 1;   // ok
}
